...
`IntrLine2Triangle2`クラスは、2次元空間における線（無限に続く直線）と三角形の交差を判定するためのクラスです。
...
`Line2d` と `Triangle2d` オブジェクトを指定して、`IntrLine2Triangle2` のインスタンスを作成します。
...
IntrLine2Triangle2 intersector = new IntrLine2Triangle2(line, triangle);
```
...
- `Param0`, `Param1`: 線上における交差点までのパラメータ
